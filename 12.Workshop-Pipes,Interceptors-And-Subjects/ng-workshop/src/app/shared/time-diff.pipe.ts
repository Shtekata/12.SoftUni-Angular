...
export class TimeDiffPipe implements PipeTransform {
  ...
  transform(value: string): unknown {
    ...
    const msPerMonth = msPerDay * 30.42;
    ...
    if (elapsed < msPerMonth) { return rtf.format(-Math.floor(elapsed / msPerDay), 'days') };
    if (elapsed < msPerYear) { return rtf.format(-Math.floor(elapsed / msPerMonth), 'months') };
